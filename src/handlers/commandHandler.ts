  ...
  async execute(client) {
    ...
    for (const folder of commandFolders) {
      ...
      for (const file of commandFiles) {
        const command: ICommand = await require(`../commands/${folder}/${file}`);
        ...
        if (!name) {
          client.logger.error(`Het commando in "${file}" heeft geen naam ingesteld.`);
          commandError = true;
        }

        // Check if there is an invalid alias
        if (aliases?.includes('')) {
          client.logger.error(`Het commando "${name}" heeft een ongeldige alias ingesteld.`);
          commandError = true;
        }

        // Check if the command has a description
        if (!description) {
          client.logger.warn(`Het commando "${name}" heeft geen beschrijving ingesteld.`);
        }

        // Check that the cooldown is not lower than 0
        if ((cooldown || 0) < 0) {
          client.logger.error(`Het commando "${name}" heeft de cooldown lager ingesteld dan 0.`);
          commandError = true;
        }

        // Check if the command has a category
        // if (!category) {
        //   client.logger.warn(`Het commando "${name}" heeft geen categorie ingesteld.`);
        // }

        // Check that the minimum arguments are not lower than 0
        if ((minArgs || 0) < 0) {
          client.logger.error(`Het commando "${name}" heeft het minimum aantal argumenten lager ingesteld dan 0.`);
          commandError = true;
        }
        ...
        if (maxArgs < (minArgs || 0)) {
          if (maxArgs < 0) {
            client.logger.error(`Het commando "${name}" heeft het maximum aantal argumenten lager ingesteld dan 0.`);
            commandError = true;
          } else {
            client.logger.error(`Het commando "${name}" heeft het maximum aantal argumenten lager ingesteld dan het minimum aantal.`);
            commandError = true;
          }
        }
        ...
        if (maxArgs > 0) {
          if (!expectedArgs?.length) {
            client.logger.error(`Het commando "${name}" heeft de eigenschap "maxArgs" ingesteld zonder de eigenschap "expectedArgs".`);
            commandError = true;
          } else if (maxArgs !== expectedArgs?.length) {
            client.logger.error(`Het commando "${name}" heeft een incorrect aantal "expectedArgs" ingesteld ten opzichte van "maxArgs". Er werden ${expectedArgs?.length} argumenten ingesteld, maar verwachte er ${maxArgs}.`);
            commandError = true;
          }
        }
        ...
        if (expectedArgs?.includes('')) {
          client.logger.error(`Het commando "${name}" heeft een ongeldige "expectedArgs" ingesteld.`);
          commandError = true;
        }

        // Check if the command has an error message when it needs it
        if ((minArgs || 0) > 0 && !syntaxError) {
          client.logger.error(`Het commando "${name}" heeft de eigenschap "minArgs" ingesteld zonder de eigenschap "syntaxError".`);
          commandError = true;
        }
        ...
        if (examples?.length) {
          if (examples.includes('')) {
            client.logger.error(`Het commando "${name}" heeft een ongeldig voorbeeld ingesteld.`);
            commandError = true;
          }
        } else {
          client.logger.warn(`Het commando "${name}" heeft geen voorbeelden ingesteld.`);
        }

        // If the command isn't a slash command, don't allow options
        if (!slash && options) {
          client.logger.error(`Het commando "${name}" heeft de eigenschap "options" ingesteld terwijl het geen slash commando is.`);
          commandError = true;
        }
        ...
        if (slash) {
          ...
          if (!description) {
            client.logger.error(`Een beschrijving is vereist voor het commando "${name}" omdat het een slash commando is.`);
            commandError = true;
          }

          // Check that the maximum arguments are not greater than 25 if it's a slash command
          if (maxArgs > 25) {
            client.logger.error(`Het slash commando "${name}" heeft een maximum aantal argumenten groter dan 25 ingesteld.`);
            commandError = true;
          }
