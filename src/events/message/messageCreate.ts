...
import { ProfileDoc, profileModel } from '../../models/profileModel';
import { IEvent } from '../../structures/IEvent';
import { formatToDuration } from '../../utils/functions';
...
export = {
  ...
  async execute(client, message: Message<true>) {
    ...
    let userData: ProfileDoc;
    try {
      userData = await profileModel.findOne({ _id: message.author.id }) ?? await profileModel.create({ _id: message.author.id });
    } catch (error) {
      client.logger.error('Er is een fout opgetreden bij het toevoegen van een gebruiker aan de database.', error);
    ...
    if (command.permissions?.length) {
      const invalidUserPerms: PermissionString[] = [];
      const invalidBotPerms: PermissionString[] = [];
    ...
    if (command.cooldown) {
      const cooldown = userData.cooldowns.find(cd => cd.guildId === message.guild.id && cd.name === command.name);
      ...
      if (cooldown) {
        ...
        userData.cooldowns = userData.cooldowns.filter(cd => cd.guildId !== message.guild.id || cd.name !== command.name);
        await userData.save();
      }

      // Add a cooldown for the user
      userData.cooldowns.push({
    ...
    try {
      ...
      client.logger.error(`Er is een fout opgetreden bij het uitvoeren van het commando '${command.name}'.`, error);
